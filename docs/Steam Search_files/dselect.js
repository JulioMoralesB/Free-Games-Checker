...
function DSelectShow( id )
{
	...
	// Widen the droplist if it would need a horizontal scrollbar. Do this after applying `$rgDroplistCSS` since that
	// can change the size of the droplist.
	if ( !bUseGamepadScreenMode && $Droplist.length )
	{
		// Remove any previously-set width to ensure it doesn't affect the `scrollWidth` calculation.
		$Droplist.removeAttr( 'style' );

		// Use `.innerWidth()` (and not `.width()`) here since that includes padding, which matches `.scrollWidth`.
		const nScrollWidth = $Droplist[0].scrollWidth;
		if ( nScrollWidth > $Droplist.innerWidth() )
		{
			$Droplist.innerWidth( nScrollWidth + 10 );
		}
	}
